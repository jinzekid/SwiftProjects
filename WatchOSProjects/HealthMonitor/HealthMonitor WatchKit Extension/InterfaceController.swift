...
class InterfaceController: WKInterfaceController {
    ...
    // Define our HealthKit Store for loading and saving object data
    ...
    override func willActivate() {
        ...
        guard HKHealthStore.isHealthDataAvailable() == true else {
            self.showDialog("HealthKit", message:"HealthKit not supported", buttonText:"Ok")
            return
        }
        ...
        let healthKitTypesToRead = NSSet(objects:
            ...
            HKQuantityType.quantityTypeForIdentifier(HKQuantityTypeIdentifierStepCount)!) as!
                Set<HKObjectType>
        healthKitStore.requestAuthorizationToShareTypes(nil,
                                                        readTypes: healthKitTypesToRead) {
                                                            (success, error) in
                                                            if success == false {
                                                                self.showDialog("HealthKit", message: "HealthKit not allowed", buttonText: "OK")
                                                            }
        }
    }
    ...
    func showDialog(title: String, message: String, buttonText: String) {
        let buttonAction = WKAlertAction(title: buttonText, style: .Cancel) {
        }
        presentAlertControllerWithTitle(title, message: message, preferredStyle: .ActionSheet, actions: [buttonAction])
    }
    ...
    func heartRateStreamingQuery(workoutStartDate: NSDate) -> HKQuery? {
        ...
        let heartRateQuery = HKAnchoredObjectQuery(type: heartRateSample, predicate: nil, anchor: anchor, limit: Int(HKObjectQueryNoLimit), resultsHandler: {(query, sampleObjects, deleteObjects, newAnchor, error) in
    ...
    func dailyStepsStreamingQuery(workoutStartDate: NSDate) -> HKQuery? {
        ...
        let startDate = calendar.dateByAddingUnit(.Day, value: -1, toDate: workoutStartDate, options: .WrapComponents)
        
        let predicate = HKQuery.predicateForSamplesWithStartDate(startDate, endDate: workoutStartDate, options: .StrictEndDate)
        let dailyStepsQuery = HKSampleQuery(sampleType: dailyStepsSample, predicate: predicate, limit: 0, sortDescriptors: nil) { (query, results, error) in
            ...
            dispatch_async(dispatch_get_main_queue(), { 
                ...
                for steps in dailyStepsSamples {
                    dailySteps += steps.quantity.doubleValueForUnit(HKUnit.countUnit())
                }
